...
public class Lucky extends Thread {
    ...
    protected synchronized void joinLuck(Player p, int joinxu) throws IOException {
        ...
        if (joinxu >= 10000000 && p.c.isTaskDanhVong == 1 && p.c.taskDanhVong[0] == 0 && p.c.taskDanhVong[1] < p.c.taskDanhVong[2]) {
            p.c.taskDanhVong[1]++;
    ...
    private void turned() throws Exception {
        ...
        Manager.serverChat("Thông Báo", "Chúc mừng " + p.name.toUpperCase() + " đã chiến thắng " + Util.getFormatNumber(xuwin) + " xu trong trò chơi Vòng xoay may mắn");
