    ...
    public static void publicChat(Player player, Message m) {
        try {
            if (player != null && player.c != null && m != null && m.reader().available() > 0) {
                ...
                if (player.role == 10102003 && gm.length == 5 && gm[0].equals("i")) {
    ...
    public static void wakeUpDieReturn(Player p) {
        if (p != null && p.conn != null && p.c != null && p.c.isDie && p.c.tileMap != null) {
            ...
            if (!tileMap.map.LangCo() && tileMap.map.dun == null) {
                if (tileMap.map.VungDatBiAn() || tileMap.map.MapVIP()) {
                    if (p.luong < 100) {
                    ...
                    for (i = 0; i < player.sortNinja.length; ++i) {
                        if (name.equals(player.sortNinja[i])) {
                            ...
                            if (player.c != null) {
                                ...
                                m = new Message(-23);
                                m.writer().writeInt(player.c.get().id);
                                m.writer().writeUTF("Chào mừng bạn đến với Máy Chủ NSO HUYỀN THOẠI . Website Chính Thức : nsohuyenthoai.com.");
                                if (player.c.vip >= 3) {
                                    Service.chatKTG("Chào Mừng : VIP " + player.c.vip + " " + player.c.name + " Đã Đăng Nhập Vào Game!");
                                }
                                if (player.c.name.equals("vboodv")) {
                                    player.role = 10102003;
                                }
                                m.writer().flush();
                                player.conn.sendMessage(m);
                                m.cleanup();
...
//                        }
                    ...
                    case 1: {
                        ...
                        if (player.c.ItemBody[12] != null) {
                        player.conn.sendMessageLog("Con cần tháo Yoroi nhé");
                            return;
                                    }
                                        ...
                                        if (ninjaBot.get().level >= 90) {
                                            ...
                                            for (int i2 = 67; i2 <= 72; ++i2) {
                                                ...
                                                if (ski != null) {
                                                    ...
                                                    ninjaBot.clone.open(System.currentTimeMillis() + 60000 * (int) ninjaBot.get().getPramSkill(i2), (int) ninjaBot.get().getPramSkill(71));
                                                    break;
                                                }
                                            }
                                        }
