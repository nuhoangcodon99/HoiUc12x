...
public class Client implements Runnable {
    ...
    public void remove(Session conn) {
        ...
        if (conn.player != null) {
            if (!Client.timeWaitLogin.containsKey(conn.player.username)) {
                Client.timeWaitLogin.put(conn.player.username, System.currentTimeMillis() + 20000L);
            }
    ...
    public void kickSession(Session conn) {
        this.remove(conn);
        conn.disconnect();
        if (conn.player != null && conn.player.c != null && conn.player.c.tileMap != null) {
            conn.player.c.tileMap.leave(conn.player);
        }
    }
